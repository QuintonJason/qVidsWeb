...
function playList(options, arg) {
  ...
  player.pl._setVideo = function(index) {
    if (index < player.pl.videos.length) {
      ...
      if (player.pl.getVideoSource) {
        player.pl.getVideoSource(player.pl.videos[index], function(
          src,
          poster
        ) {
        ...
        player.pl._setVideoSource(
          player.pl.videos[index].src,
          player.pl.videos[index].poster
        );
      }
    }
  };
  ...
  if (options instanceof Array) {
    ...
    typeof player.pl[options] !== "undefined"
  ) {
...
function getVideos(ele) {
  videos = [];
  var nodesArray = [].slice.call(
    document.querySelectorAll("ol.sortable-list .btn")
  );
  var nodesArray;
...
var demoModule = {
  ...
  createListOfVideos: function() {
    ...
    for (var i = 0, len = this.player.pl.videos.length; i < len; i++) {
      html +=
        '<li data-videoplaylist="' +
        i +
        '">' +
        '<span class="number">' +
        (i + 1) +
        "</span>" +
        '<span class="poster"><img src="' +
        videos[i].poster +
        '"></span>' +
        '<span class="title">' +
        videos[i].title +
  ...
  updateActiveVideo: function() {
    ...
    this.els.$playlist
      .find('li[data-videoplaylist="' + activeIndex + '"]')
      .addClass("active");
  },
  bindEvents: function() {
    var self = this;
